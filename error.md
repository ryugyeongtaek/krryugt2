...
## Agent 컨텍스트 길이 초과 (2026-09-10)

### 증상

전체 출고 추이와 수요 프로파일을 묻는 질문에서 LLM이 128,000 토큰 한도를 초과했다는 `context_length_exceeded` 오류가 발생했습니다. 조회 결과가 약 440,986 토큰으로 전달되었습니다.

### 원인

Agent Tool이 전체 품목 행을 그대로 대화 이력에 넣어, 화면 조회에는 필요하지만 LLM 답변에는 과도한 결과가 전달되었습니다.

### 해결

Agent 전용 Tool 결과 배열을 최대 100행으로 제한하고 제한된 결과만 숫자 근거 사전에 포함하도록 수정했습니다. 화면 조회 함수와 DB 계산은 변경하지 않았습니다.

## Agent 최종 답변 파싱 실패 (2026-09-10)

### 증상

Agent가 데이터를 조회한 뒤에도 `INVALID_LLM_ANSWER`를 표시했습니다.

### 원인

최종 LLM 응답이 프로젝트의 엄격한 `AgentAnswer` 필드 계약과 일치하지 않으면, JSON 코드블록처럼 내용은 JSON이어도 기존 파서가 응답 전체를 JSON으로 해석하지 못했습니다. JSON fallback 요청의 출력 형식 지시도 필드 계약까지 구체적으로 안내하지 않았습니다.

### 해결

전체 응답이 하나의 JSON 코드블록인 경우에만 내부 JSON을 파싱하도록 보완하고, 필수 필드 누락·추가 필드·잘못된 JSON 검증은 그대로 유지했습니다. `json_object` 요청에는 필수 `AgentAnswer` 필드와 코드블록 금지 지시를 함께 전달하도록 수정했습니다.

### 추가 조치

호환 모델이 첫 최종 응답에서 여전히 `AgentAnswer` 형식을 지키지 않는 경우를 위해, 형식 오류를 한 번만 명시적으로 재요청하도록 보완했습니다. 재요청도 실패하면 `INVALID_LLM_ANSWER`로 종료하며 임의의 답변을 표시하지 않습니다.
